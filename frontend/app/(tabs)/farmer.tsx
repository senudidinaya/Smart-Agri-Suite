...
export default function FarmerDashboard() {
  ...
  const profitTrend = useMemo(() => {
    ...
    return {
      labels,
      datasets: [{ data: profits.length ? profits : [0] }],
    };
  }, [orders]);
  ...
  const profitForecast = useMemo(() => forecastProfitTrend(orders as any), [orders]);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#F8FAFC" }}>
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <Animated.View style={styles.gaugeRow} entering={FadeInDown.delay(staggerDelay * 3).springify()}>
          <View style={styles.gaugeItem}>
            <AnimatedCircularProgress
              ...
              duration={1500}
            >
              {() => <Text style={styles.gaugeCenterText}>{Math.round(totalRevenue / 1000)}K</Text>}
            </AnimatedCircularProgress>
            <Text style={styles.gaugeLabel}>Revenue</Text>
          </View>
        ...
        {/* ACTIVE ORDERS LIST */}
        <Animated.View entering={FadeInDown.delay(staggerDelay * 4).springify()} style={{ marginTop: 8 }}>
           <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16}}>
               <Text style={styles.sectionTitleWithoutMargin}>Active Orders</Text>
           ...
           {activeOrders.length === 0 ? (
               <Text style={{fontFamily: 'Poppins_400Regular', color: '#64748B', marginBottom: 20}}>No active pipeline.</Text>
           ) : (
             activeOrders.map((order, i) => {
                 const id = String(order._id || order.id);
                 return (
                  <Animated.View key={id} entering={FadeInDown.delay(staggerDelay * (4 + i)).springify()} style={styles.orderCard}>
                      <View style={styles.orderHeader}>
                         <View style={{flexDirection: 'row', alignItems: 'center'}}>
                            <View style={styles.orderIconWrap}>
                                <Ionicons name="cube" size={16} color="#3B82F6" />
                            </View>
                            <Text style={styles.orderTitle}>{order.spice} ({order.quantity}kg)</Text>
                         </View>
                         {updatingId === id ? (
                             ...
                             >
                                <View style={[styles.statusPill, order.status === "PENDING" ? {backgroundColor: '#FEF3C7'} : order.status === "CONFIRMED" ? {backgroundColor: '#E0E7FF'} : {backgroundColor: '#F3E8FF'}]}>
                                    <Text style={[styles.statusPillText, order.status === "PENDING" ? {color: '#D97706'} : order.status === "CONFIRMED" ? {color: '#4338CA'} : {color: '#7E22CE'}]}>
                                        {order.status}
                                    </Text>
                                </View>
                             </Pressable>
                      ...
                      <View style={styles.orderMeta}>
                          <View>
                              <Text style={styles.metaLabel}>Target Market</Text>
                              <Text style={styles.metaValue}>{order.customer}</Text>
                          </View>
                          <View style={{alignItems: 'flex-end'}}>
                              <Text style={styles.metaLabel}>Net Output</Text>
                              <Text style={[styles.metaValue, {color: '#10B981', fontFamily: 'Poppins_700Bold'}]}>
        ...
        <Animated.View style={styles.card} entering={FadeInDown.delay(staggerDelay * 4).springify()}>
          ...
          <Text style={styles.sectionSub}>Historical profit mapping across recent orders.</Text>
        ...
        <Animated.View style={styles.card} entering={FadeInDown.delay(staggerDelay * 5).springify()}>
          ...
          {demandData.length === 0 ? (
            <Text style={{fontFamily: 'Poppins_400Regular', color: '#64748B', marginTop: 12}}>No orders yet.</Text>
          ) : (
            <PieChart
              data={demandData}
        ...
        {/* MARKET HEATMAP */}
        <Animated.View style={styles.card} entering={FadeInDown.delay(staggerDelay * 6).springify()}>
          <Text style={styles.sectionTitle}>Market Opportunity Heatmap</Text>
          <Text style={styles.sectionSub}>Live regional demand index.</Text>
        ...
        <Animated.View style={styles.card} entering={FadeInDown.delay(staggerDelay * 7).springify()}>
          ...
          <View style={styles.insightBox}>
             ...
             <Text style={styles.insightText}>
               Cinnamon price expected to jump{" "}
               <Text style={{fontFamily: 'Poppins_700Bold', color: '#10B981'}}>{priceForecast.growthRate.toFixed(1)}%</Text>
             </Text>
          </View>
          ...
          <View style={styles.insightBox}>
             ...
             <Text style={styles.insightText}>
               Target price: LKR <Text style={{fontFamily: 'Poppins_700Bold', color: '#0F172A'}}>{priceForecast.predictedPrice.toFixed(0)}</Text>
             </Text>
          </View>
          ...
          <View style={styles.insightBox}>
             ...
             <Text style={styles.insightText}>
               Profit projection shift: <Text style={{fontFamily: 'Poppins_700Bold', color: '#8B5CF6'}}>{profitForecast.growthRate.toFixed(1)}%</Text>
             </Text>
          </View>

          {demandForecast.slice(0, 2).map((d, i) => (
             <View key={d.district} style={[styles.insightBox, i === 1 && {borderBottomWidth: 0, paddingBottom: 0}]}>
                <Ionicons name="location" size={16} color="#F59E0B" />
                <Text style={styles.insightText}>
                  Rising demand mapped in <Text style={{fontFamily: 'Poppins_600SemiBold'}}>{d.district}</Text>
                </Text>
             </View>
          ))}
        </Animated.View>
        ...
        <Animated.View style={styles.tipCard} entering={FadeInDown.delay(staggerDelay * 8).springify()}>
          <View style={styles.tipHeaderRow}>
            <Ionicons name="bulb" size={20} color="#fff" />
            <Text style={styles.tipTitle}>Smart Market Move</Text>
          </View>

          {marketRecommendation && (
            <View style={{marginTop: 8}}>
              <Text style={styles.tipLabel}>Optimal Target Market</Text>
              <Text style={styles.tipValue}>
                {marketRecommendation.district} via {marketRecommendation.transportMode}
              </Text>
              
              <View style={styles.divider} />
              
              <Text style={styles.tipLabel}>Projected Yield</Text>
...
const styles = StyleSheet.create({
  ...
  tipCard: {
    backgroundColor: "#2E7D32", // Assuming we want to keep a strong green here for "Smart Action"
    borderRadius: 20,
    padding: 20,
    marginBottom: 16,
    shadowColor: "#2E7D32",
  ...
  tipTitle: { color: "white", fontFamily: "Poppins_600SemiBold", fontSize: 16, marginLeft: 8 },
  tipLabel: { color: "rgba(255,255,255,0.8)", fontFamily: "Poppins_400Regular", fontSize: 13, marginTop: 12 },
  tipValue: { color: "white", fontFamily: "Poppins_600SemiBold", fontSize: 15, marginTop: 2 },
  tipValueLarge: { color: "white", fontFamily: "Poppins_700Bold", fontSize: 24, marginTop: 4 },
